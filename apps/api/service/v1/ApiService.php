<?php namespace apps\api\service\v1;
...
use apps\common\service\AuthUcService;
...
class ApiService {
  ...
  public function signature( $inputArr ) {
    ...
    $signature = implode( '&' , $new_arr ) . '&secret=' . config( 'custom.Api' )['secret'];
  ...
  public function validTimestamp( $timestamp ) {
    ...
    return abs( time() - $timestamp ) < config( 'custom.Api' )['timeGap'];
  }
