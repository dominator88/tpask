...
class BindphoneService extends ApiService {
  ...
  public static function instance( $params = [] ) {
    if ( self::$instance == NULL ) {
      self::$instance         = new BindphoneService();
  ...
  public function post() {
    ...
    if ( ! $SysSms->validCaptcha( $this->params['phone'] , $this->params['captcha'] ) ) {
      return api_result( '验证码不正确' , 500 );
    }
    
    $MerUser = MerUserService::instance();
    
    return $MerUser->bindPhone( $this->params['merId'] , $this->userId , $this->params['phone'] );
  }
