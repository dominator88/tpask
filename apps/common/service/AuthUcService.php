...
class AuthUcService {
  ...
  public static function instance() {
    if ( self::$instance == NULL ) {
      self::$instance = new AuthUcService();
    }
  ...
  function getByToken($token){
      $user = db('ecm_access_token');
      return $user->where('token',$token)->find();
  }
  ...
  function ucSignIn($username,$password){
      if(empty($username)){
          return ajax_arr( '用户名或手机不能为空' , 500 );
      }
      if(empty($password)){
     ...
     $url = 'http://www.51zwd.com/index.php?app=member&act=loginApi';
